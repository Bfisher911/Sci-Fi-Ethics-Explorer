...
// Lazy-load CinematicHero. It's the heaviest single component on the
// dashboard (layered gradients, big inline styles, ~350 lines of JSX
// + the Resume Reading inset). Pulling it out of the critical bundle
...
const CinematicHero = dynamic(
  () =>
    import('@/components/dashboard/cinematic-hero').then((m) => m.CinematicHero),
  {
    ssr: false,
    loading: () => (
      <div
        className="rounded-2xl border bg-card/30 backdrop-blur"
        style={{
          minHeight: 460,
          borderColor: 'hsl(var(--border) / 0.5)',
          background:
            'linear-gradient(135deg, hsl(var(--card) / 0.4) 0%, hsl(var(--sidebar-background) / 0.4) 100%)',
        }}
      />
    ),
  },
