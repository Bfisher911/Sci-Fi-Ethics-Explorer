...
/**
 * Directory view — the body of the legacy /directory page, extracted
 * so it can be embedded inside /people's tab system. Outer "People"
 * hero card removed (the /people page wears that hat).
 *
 * Includes super-admin controls (impersonation + community-manager
 * grant/revoke) so the consolidated /people surface is a 1:1 functional
 * replacement for /directory.
 */
      ...
      <CardContent className="p-6 flex flex-col items-center gap-3 text-center">
        <Avatar className="h-20 w-20 border border-border">
          ...
          <AvatarFallback className="bg-primary/20 text-primary text-xl">
            {user.avatarUrl ? initial : <UserCircle className="h-10 w-10" aria-hidden />}
          </AvatarFallback>
        </Avatar>
        <div className="space-y-1 min-w-0 w-full">
          ...
          {user.dominantFramework ? (
            ...
            <p className="text-xs text-muted-foreground italic">No framework yet</p>
          )}
...
export function DirectoryView(): React.ReactElement {
  ...
  const handleToggleManager = async (
    ...
    try {
      ...
      if (res.success) {
        ...
        setAllUsers((prev) =>
          prev.map((u) =>
            u.uid === targetUid ? { ...u, communityManager: !currentlyManager } : u,
          ),
        );
      } else {
        toast({
