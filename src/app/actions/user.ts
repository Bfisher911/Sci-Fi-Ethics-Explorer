...
export async function getUserProfile(uid: string): Promise<UserProfile | null> {
  ...
  console.log(`Attempting to fetch profile for UID: ${uid} from 'users' collection`);
  try {
    const userDocRef = doc(db, 'users', uid); // Changed back to 'users'
    const userDoc = await getDoc(userDocRef);

    if (userDoc.exists()) {
      const data = userDoc.data();
      console.log(`Raw profile data found for UID ${uid} in 'users' collection:`, JSON.stringify(data));
      ...
      console.log(`Processed profile for UID ${uid} from 'users' collection:`, JSON.stringify(profile));
      return profile;
    } else {
      console.log(`No profile document found for UID: ${uid} in 'users' collection.`);
      return null;
    }
  } catch (error) {
    console.error(`Error fetching user profile from Firestore for UID ${uid} in 'users' collection:`, error);
...
export async function updateUserProfile(uid: string, data: Partial<UserProfile>): Promise<void> {
  ...
  console.log(`Attempting to update/create profile for UID: ${uid} in 'users' collection with data:`, JSON.stringify(data));
  try {
    const userDocRef = doc(db, 'users', uid); // Changed back to 'users'
    ...
    console.log(`Successfully updated/created profile for UID: ${uid} in 'users' collection`);
  } catch (error) {
    console.error(`Error updating/creating user profile for UID ${uid} in 'users' collection:`, error);
    const specificErrorMessage = error instanceof Error ? error.message : String(error);
...
export async function createUserProfile(uid: string, email: string | null, displayName?: string, avatarUrl?: string): Promise<void> {
  ...
  console.log(`Attempting to create profile for UID: ${uid} in 'users' collection`);
  try {
    const userDocRef = doc(db, 'users', uid); // Changed back to 'users'
    const now = serverTimestamp();

    // Try to parse firstName and lastName from displayName if available
    ...
    console.log(`Successfully created profile for UID: ${uid} in 'users' collection with data:`, JSON.stringify(initialProfileData).replace(/"now"/g, "serverTimestamp()"));
  } catch (error) {
    console.error(`Error creating user profile for UID ${uid} in 'users' collection:`, error);
    const specificErrorMessage = error instanceof Error ? error.message : String(error);
