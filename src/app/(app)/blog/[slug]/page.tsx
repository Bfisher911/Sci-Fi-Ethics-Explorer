...
export default function BlogPostPage() {
  ...
  useEffect(() => {
    getBlogPostBySlug(slug).then((res) => {
      if (res.success) {
        // Community posts canonically live under /community-blog/[slug].
        // If someone hits the official URL with a community slug, redirect.
        if (
          res.data &&
          (res.data.kind === 'community' ||
            (!res.data.kind && !isOfficialAuthor(res.data.authorId)))
        ) {
          router.replace(`/community-blog/${res.data.slug}`);
          return;
        }
        setPost(res.data);
      }
  ...
  return (
    <div className="container mx-auto py-8 px-4 max-w-3xl">
      ...
      <article className="mt-4">
        <Card className="bg-card/80 backdrop-blur-sm border-l-4 border-l-primary/60">
          <CardContent className="p-6 md:p-10 space-y-6">
            <div className="space-y-3">
              <BlogKindBadge kind={post.kind || 'official'} authorId={post.authorId} />
